...
async def run_aqm_announcer(bus: Any, cfg: Any) -> None:
    ...
    log.info(base + ["audio_dir"])
    # Load config
    audio_dir = str(_cfg_get(cfg, base + ["audio_dir"], "styffioCoolness"))

    log.info(f"Using audio directory: {audio_dir}")
